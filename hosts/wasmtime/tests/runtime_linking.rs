...
fn runtime_linked_plan_runs_consumer_with_bound_provider() {
    let (Ok(consumer), Ok(provider)) = (
        std::fs::read(example("dynlink-endpoint-consumer", "dynlink-endpoint-consumer.wasm")),
        std::fs::read(example("dynlink-echo-provider", "dynlink_echo_provider.wasm")),
    ) else {
        eprintln!(
    ...
    let plan = PlanV1 {
        ...
        components: vec![
            ComponentSpec { id: "consumer".to_string(), digest: consumer_digest, source: None },
            ComponentSpec { id: "provider".to_string(), digest: provider_digest, source: None },
        ],
...
fn runtime_linked_plan_rejects_untrusted_provider() {
    ...
    assert_eq!(err.code, compose_host_wasmtime::ErrorCode::TrustUntrustedSource);
}
